<?php 
  // error_reporting(0);
  session_start();

  $usernameD = 'root';
  $passwordD = '';
  $hostnameD = 'localhost';
  $databaseD = 'social_db';
  $portD = '3306';

  $connect = mysqli_connect(username: $usernameD,
              password: $passwordD,
              hostname: $hostnameD,
              database: $databaseD,
              port: $portD
            );

  $currentUserId = $_SESSION['userNo'];

  function refreshRequests(){
    header('Location: friends.php');
  }

  function sendRequest($sid, $rid, $sname, $rname, $spfp, $rpfp, $conn){
    $sqlRequest = "INSERT INTO tbl_social_friends(sID, rID, sname, rname, spfp, rpfp, status) VALUES ('$sid','$rid','$sname','$rname','$spfp','$rpfp','pending')";

    mysqli_query($conn, $sqlRequest);
    refreshRequests();
  }

  function accepted($sid, $rid, $conn){
    $sqlAccept = "UPDATE tbl_social_friends SET status = 'accepted' WHERE (sID = '$sid' AND rID = '$rid')";

    mysqli_query($conn, $sqlAccept);
    refreshRequests();
  }

  function rejected($sid, $rid, $conn){
    $sqlReject = "DELETE FROM tbl_social_friends WHERE (sID = '$sid' AND rID = '$rid')";

    mysqli_query($conn, $sqlReject);
    refreshRequests();
  }

  if(isset($_GET['modes']) == 'request'){
  	unset($_GET['mod']);
  	unset($_GET['modo']);
    sendRequest($_SESSION['userNo'], $_GET['rid'], $_SESSION['userID'], $_GET['rname'], $_SESSION['userPfp'], $_GET['rpfp'], $connect);
  }

  if(isset($_GET['mod']) == 'accept'){
  	unset($_GET['modes']);
  	unset($_GET['modo']);
    accepted($_GET['sid'], $currentUserId, $connect);
  }

  if(isset($_GET['modo']) == 'reject'){
  	unset($_GET['modes']);
  	unset($_GET['mod']);
    rejected($_GET['sid'], $currentUserId, $connect);
  }

  if($connect){
    $sql = "SELECT * FROM tbl_posts";
    $sqlUsr = "SELECT * FROM tbl_social_users";
    $sqlFriends = "SELECT * FROM tbl_social_friends WHERE rID = '$currentUserId' AND status = 'pending'";
    $sqlFriendsAcc = "SELECT * FROM tbl_social_friends WHERE (rID = '$currentUserId' OR sID = '$currentUserId') AND status = 'accepted'";

    $query = $connect->query($sql);
    $queryUsr = $connect->query($sqlUsr);
    $queryFriends = $connect->query($sqlFriends);
    $queryFriendsAcc = $connect->query($sqlFriendsAcc);
  }
?>

<?php include_once('bs-links.php'); ?>

<!DOCTYPE html>
<html lang="en">
  
  <head>
    <meta charset="utf-8">
    <title>
      Quick Meet - Friends
    </title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@4.4.1/dist/css/bootstrap.min.css" rel="stylesheet">
    <link href="css/cs-home.css" rel="stylesheet">
  </head>
  
  <body>
    <div class="container">
      <div class="profile-page tx-13">
        <div class="row">
          <div class="col-12 grid-margin">
            <div class="profile-header">
              <div class="cover">
                <div class="gray-shade">
                </div>
                <figure>
                  <?php if(!empty($_SESSION['userBack'])) { ?>
                    <img src="./background/<?php echo $_SESSION['userBack']; ?>" class="img-fluid" alt="profile cover">
                  <?php }else{ ?>
                    <img src="https://bootdey.com/img/Content/bg1.jpg" class="img-fluid" alt="profile cover">
                  <?php } ?>
                </figure>
                <div class="cover-body d-flex justify-content-between align-items-center">
                  <div>
                    <?php if(!empty($_SESSION['userPfp'])) { ?>
                      <img class="profile-pic" src="./pfp/<?php echo $_SESSION['userPfp']; ?>"
                      alt="https://bootdey.com/img/Content/avatar/avatar6.png">
                    <?php }else{ ?>
                      <img class="profile-pic" src="https://bootdey.com/img/Content/avatar/avatar6.png" 
                      alt="https://bootdey.com/img/Content/avatar/avatar6.png">
                    <?php } ?>
                    <span class="profile-name">
                    	<?php echo $_SESSION['userID']; ?>
                    </span>
                  </div>
                  <div class="d-none d-md-block">
                    <button class="btn btn-danger btn-icon-text btn-edit-profile">
                      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                      fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                      stroke-linejoin="round" class="feather feather-edit btn-icon-prepend">
                        <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7">
                        </path>
                        <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z">
                        </path>
                      </svg>
                      <a href = "user-edit.php" class="text-white">Edit profile</a>
                    </button>
                  </div>
                </div>
              </div>
              <div class="header-links">
                <ul class="links d-flex align-items-center mt-3 mt-md-0">
                  <li class="header-link-item d-flex align-items-center">
                    <a href="home.php">
                      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                      fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                      stroke-linejoin="round" class="feather feather-columns mr-1 icon-md">
                        <path d="M12 3h7a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-7m0-18H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h7m0-18v18">
                        </path>
                      </svg>
                    </a>
                    <a class="pt-1px d-none d-md-block" href="home.php">
                      Timeline
                    </a>
                  </li>
                  <li class="header-link-item ml-3 pl-3 border-left d-flex align-items-center">
                    <a href="about.php">
                      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                      fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                      stroke-linejoin="round" class="feather feather-user mr-1 icon-md">
                        <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2">
                        </path>
                        <circle cx="12" cy="7" r="4">
                        </circle>
                      </svg>
                    </a>
                    <a class="pt-1px d-none d-md-block" href="about.php">
                      About
                    </a>
                  </li>
                  <li class="header-link-item ml-3 pl-3 border-left d-flex align-items-center active">
                    <a href="friends.php">
                      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                      fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                      stroke-linejoin="round" class="feather feather-users mr-1 icon-md">
                        <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2">
                        </path>
                        <circle cx="9" cy="7" r="4">
                        </circle>
                        <path d="M23 21v-2a4 4 0 0 0-3-3.87">
                        </path>
                        <path d="M16 3.13a4 4 0 0 1 0 7.75">
                        </path>
                      </svg>
                    </a>
                    <a class="pt-1px d-none d-md-block" href="#">
                      Friends
                      <span class="text-muted tx-12">
                        <?php echo $_SESSION['userFriends']; ?>
                      </span>
                    </a>
                  </li>
                  <li class="header-link-item ml-3 pl-3 border-left d-flex align-items-center">
                    <a href="photos.php">
                      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                      fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                      stroke-linejoin="round" class="feather feather-image mr-1 icon-md">
                        <rect x="3" y="3" width="18" height="18" rx="2" ry="2">
                        </rect>
                        <circle cx="8.5" cy="8.5" r="1.5">
                        </circle>
                        <polyline points="21 15 16 10 5 21">
                        </polyline>
                      </svg>
                    </a>
                    <a class="pt-1px d-none d-md-block" href="photos.php">
                      Photos
                    </a>
                  </li>
                  <li class="header-link-item ml-3 pl-3 border-left d-flex align-items-center">
                    <a href="videos.php">
                      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                      fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                      stroke-linejoin="round" class="feather feather-video mr-1 icon-md">
                        <polygon points="23 7 16 12 23 17 23 7">
                        </polygon>
                        <rect x="1" y="5" width="15" height="14" rx="2" ry="2">
                        </rect>
                      </svg>
                    </a>
                    <a class="pt-1px d-none d-md-block" href="videos.php">
                      Videos
                    </a>
                  </li>
                </ul>
              </div>
            </div>
          </div>
        </div>
        <div class="col-md-12 grid-margin">
          <div class="card rounded">
            <div class="card-body">
              <h6 class="card-title">
                Your Friends
              </h6>
              <?php while($requestsAcc = $queryFriendsAcc->fetch_assoc()){ ?>
              <div class="d-flex justify-content-between mb-2 pb-2 border-bottom">
                <div class="d-flex align-items-center hover-pointer">
                  <?php if($currentUserId == $requestsAcc['sID']) { ?>
                    <?php if(!empty($requestsAcc['rpfp'])) { ?>
                      <img class="img-xs rounded-circle" src="./pfp/<?php echo $requestsAcc['rpfp']; ?>"
                      alt="https://bootdey.com/img/Content/avatar/avatar6.png">
                    <?php }else{ ?>
                      <img class="img-xs rounded-circle" src="https://bootdey.com/img/Content/avatar/avatar6.png" 
                      alt="https://bootdey.com/img/Content/avatar/avatar6.png">
                    <?php } ?>
                  <?php }else{ ?>
                    <?php if(!empty($requestsAcc['spfp'])) { ?>
                      <img class="img-xs rounded-circle" src="./pfp/<?php echo $requestsAcc['spfp']; ?>"
                      alt="https://bootdey.com/img/Content/avatar/avatar6.png">
                    <?php }else{ ?>
                      <img class="img-xs rounded-circle" src="https://bootdey.com/img/Content/avatar/avatar6.png" 
                      alt="https://bootdey.com/img/Content/avatar/avatar6.png">
                    <?php } ?>
                  <?php } ?>
                  <div class="ml-2">
                    <a href="user.php?id=<?= $requestsAcc['sID']; ?>" class="text-dark">
                      <p>
                        <?php if($currentUserId == $requestsAcc['sID']) { ?>
                          <?php echo $requestsAcc['rname']; ?>
                        <?php }else{ ?>
                          <?php echo $requestsAcc['sname']; ?>
                        <?php }?>  
                      </p>
                    </a>
                    <p class="tx-11 text-muted">
                      <?php echo $_SESSION['userFriends']; ?>
                      Mutual Friends
                    </p>
                  </div>
                </div>
              </div>
            <?php } ?>
            </div>
          </div>
        </div>
        <div class="col-md-12 grid-margin">
          <div class="card rounded">
            <div class="card-body">
              <h6 class="card-title">
                Friend Requests
              </h6>
              <?php while($requests = $queryFriends->fetch_assoc()){ ?>
              <div class="d-flex justify-content-between mb-2 pb-2 border-bottom">
                <div class="d-flex align-items-center hover-pointer">
                  <?php if(!empty($requests['spfp'])) { ?>
                    <img class="img-xs rounded-circle" src="./pfp/<?php echo $requests['spfp']; ?>"
                    alt="https://bootdey.com/img/Content/avatar/avatar6.png">
                  <?php }else{ ?>
                    <img class="img-xs rounded-circle" src="https://bootdey.com/img/Content/avatar/avatar6.png" 
                    alt="https://bootdey.com/img/Content/avatar/avatar6.png">
                  <?php } ?>
                  <div class="ml-2">
                    <a href="user.php?id=<?= $requests['sID']; ?>" class="text-dark">
                      <p>
                        <?php echo $requests['sname']; ?>
                      </p>
                    </a>
                    <p class="tx-11 text-muted">
                      <?php echo $_SESSION['userFriends']; ?>
                      Mutual Friends
                    </p>
                  </div>
                </div>
                <div class="d-md-block">
                  <button class="btn btn-success btn-icon-text">
	                <a href="friends.php?mod='accept'&sid=<?= $requests['sID']; ?>" class="text-light">Accept</a>
                  </button>
                </div>
                <div class="d-md-block">
                  <button class="btn btn-danger btn-icon-text">
	                <a href="friends.php?modo='reject'&sid=<?= $requests['sID']; ?>" class="text-light">Reject</a>
                  </button>
                </div>
              </div>
            <?php } ?>
            </div>
          </div>
        </div>
        <div class="col-md-12 grid-margin">
          <div class="card rounded">
            <div class="card-body">
              <h6 class="card-title">
                Suggestions for you
              </h6>
              <?php while($users = $queryUsr->fetch_assoc()){ ?>
              <?php if($users['id'] != $_SESSION['userNo']) { ?>
              <div class="d-flex justify-content-between mb-2 pb-2 border-bottom">
                <div class="d-flex align-items-center hover-pointer">
                  <?php if(!empty($users['filename'])) { ?>
                    <img class="img-xs rounded-circle" src="./pfp/<?php echo $users['filename']; ?>"
                    alt="https://bootdey.com/img/Content/avatar/avatar6.png">
                  <?php }else{ ?>
                    <img class="img-xs rounded-circle" src="https://bootdey.com/img/Content/avatar/avatar6.png" 
                    alt="https://bootdey.com/img/Content/avatar/avatar6.png">
                  <?php } ?>
                  <div class="ml-2">
                    <a href="user.php?id=<?= $users['id']; ?>" class="text-dark">
                      <p>
                        <?php echo $users['username']; ?>
                      </p>
                    </a>
                    <p class="tx-11 text-muted">
                      <?php echo $_SESSION['userFriends']; ?>
                      Mutual Friends
                    </p>
                  </div>
                </div>
                <button class="btn btn-icon">
	                <a href="friends.php?modes='request'&rid=<?= $users['id']; ?>&rname=<?= $users['username']; ?>&rpfp=<?= $users['filename']; ?>" class="text-dark">
	                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
	                  fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
	                  stroke-linejoin="round" class="feather feather-user-plus" data-toggle="tooltip"
	                  title data-original-title="Connect">
	                    <path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2">
	                    </path>
	                    <circle cx="8.5" cy="7" r="4">
	                    </circle>
	                    <line x1="20" y1="8" x2="20" y2="14">
	                    </line>
	                    <line x1="23" y1="11" x2="17" y2="11">
	                    </line>
	                  </svg>
	                </a>
              	</button>
              </div>
            <?php } ?>
            <?php } ?>
            </div>
          </div>
        </div>
      </div>
    </div>

    <button type="button" class="btn btn-danger btn-floating btn-lg" id="btn-back-to-top">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-up" viewBox="0 0 16 16">
          <path fill-rule="evenodd" d="M8 15a.5.5 0 0 0 .5-.5V2.707l3.146 3.147a.5.5 0 0 0 .708-.708l-4-4a.5.5 0 0 0-.708 0l-4 4a.5.5 0 1 0 .708.708L7.5 2.707V14.5a.5.5 0 0 0 .5.5"/>
        </svg>
    </button>
    <script data-cfasync="false" src="/cdn-cgi/scripts/5c5dd728/cloudflare-static/email-decode.min.js">
    </script>
    <script src="https://code.jquery.com/jquery-1.10.2.min.js">
    </script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@4.4.1/dist/js/bootstrap.bundle.min.js">
    </script>
    <script src="top.js">
    </script>
    <script type="text/javascript">
    </script>
	  <script type="text/javascript">
    	/*function updateDiv(){ 
  			$('#posts').load(document.URL +  ' #posts');
  		}

		  setInterval(updateDiv, 10000);
      */
	  </script>
  </body>
</html>